    ...
    void applyState(LevelEditorLayer* editor, std::string const& uid, std::string const& data) {
        if (auto it = g_objectByUid.find(uid); it != g_objectByUid.end()) {
            ...
            if (isInLevel(object) && onlyPositionChanged(previous, data, moved)) {
                ...
                // 옮긴 결과가 상대 것과 정말 같아졌는지 확인한다.
                //
                // 이 빠른 길은 "자리만 바뀌었으니 통째로 다시 만들지 말고 옮기기만
                // 하자"는 것이다. 대부분 맞지만, 물체가 그룹에 묶여 있거나 회전/크기가
                // 걸려 있으면 옮기기만 해서는 상대와 똑같은 상태가 안 된다. 그런데도
                // "맞췄다"고 적어두면 그 어긋남은 아무도 모른 채 영원히 남는다.
                // 양쪽 개수는 같은데 자리가 다른 상태가 그렇게 만들어진다.
                //
                // 그래서 옮긴 뒤 결과를 견주어 본다. 같으면 그대로 끝내고, 다르면
                // 아래로 내려가 통째로 다시 만든다. 다시 만드는 길은 상대가 처음
                // 만들 때 쓴 길과 같아서 반드시 같은 결과가 된다.
                //
                // 지름길로 갔으면 도착지가 맞는지 확인하고, 아니면 큰길로 돌아간다.
                if (auto now = saveStringOf(object, editor); now == data) {
                    g_lastSaved[uid] = now;
                    hushObject(uid);
                    return;
                }
...
namespace coop {
    ...
    // 방의 내용을 다시 받아 내 레벨을 맞춘다.
    //
    // 한 번 어긋나면 스스로 돌아올 길이 없다. 각자 "내가 마지막으로 아는 내 모습"과
    // 견주기 때문에, 둘 다 자기는 안 변했다고 여긴다. 둘 다 자기 시계만 보고
    // 서로 시간이 맞다고 우기는 셈이다. 그럴 때 방의 시계를 다시 받아온다.
    void resyncFromRoom() {
        if (!inRoom()) return;

        // 받은 것은 g_incoming에 쌓이고, 이미 아는 uid는 그 자리에서 고쳐진다.
        // 그래서 레벨을 비우지 않아도 어긋난 것만 제자리를 찾는다.
        matjson::Value msg;
        msg["type"] = "resync";
        send(std::move(msg));
    }
